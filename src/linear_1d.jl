...
function LinearInterpolatedPDF(x, y; sorted = issorted(x))
    area = integrate(x,y)
    area ≈ 1 || error("Input is not normalized. integrate(x,y) = ", area)
    sorted || error("x is not in ascending order")

    pdf_itp = LinearInterpolation(x,y)

    cdf_y = cumul_integrate(x,y)
    cdf_y .-= first(cdf_y)
    cdf_y ./= last(cdf_y) # to fix any rounding issues i.e. 1.0000000000000002
...
"""
    fit_pdf(x::AbstractArray, s::AbstractArray)
...
# ```
# """
function fit_pdf(x::AbstractVector{XT}, s::AbstractVector{YT}) where {XT<:Real, YT<:Real}
    ...
    pdf_itp = LinearInterpolation(x, p)
